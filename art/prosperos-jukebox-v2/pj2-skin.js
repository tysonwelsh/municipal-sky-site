...
PJ2.Skin = (function () {
  ...
  // ==========================================================================
  // THE BINDING (owner 2026-08-31: PARCHMENT ONLY — the NIGHT/PARCH toggle
  // retired; the parchment page is the app's one dress). The night registry
  // above is NOT dead: it is the WINDOW ink — the canvas panels, the spiral
  // monitor and the DOM log are night windows cut into the parchment page,
  // and the viz flips MODE to "night" while drawing inside them
  // (withWindowInk). palette()/atlas()/inkAge() stay mode-aware for exactly
  // that reason; nothing user-facing switches modes anymore.
  // ==========================================================================
  var MODE = "parchment";
  var PARCH_OVERRIDES = {
    library: {
    ...
    sycorax: {
      ...
      // the old ramp was the last brown in the book). frameEdge is NOT
      // overridden: the title band wears the dusk-violet night frame tone.
    ...
    ariel: {
      ...
      worstBg: "#22345a",
      // the plate page sits one step UP the ramp on parchment; frameEdge
      // not overridden — the band keeps the steel night frame tone
  ...
  function themeCSS() {
    // parchment is the app's one dress (owner 2026-08-31) — each track gets
    // a single block of its parchment page values plus the mode-stable
    // chrome; no binding dimension.
