...
- 2026-09-25 — **Physics round 4, geometry matched to the painted bed:** 100 holes at (±0.7512, 2.30), backstop v 2.60, bedHalfW 1.02, dent 44°–69° (now worn flush, dentDepth 1.0, taper 10°). A ball propped on a 100 hole's lip in the corner pocket is not a 100: resolved with no score (`result.kind 'stuck'`). Audible `rim` ≥ 1.15 u/s.
  Bands (σ): competent 10 98 %, 30 91 %, 40 74 %, 50 19.5 % best / 6.3 % straight, 100 6.3 %, mean 340; novice 174; cup changed 27 %; rattle after landing 39 %; dent → 30 12 % (0 % elsewhere). Sweep resolve p95 4.68 s / max 6.52 s is still over 4.5 / 6.
- 2026-09-25 — **Physics round 5, the ladder holds.** `rimBallR` 0.04, the up-bed half of every rim is taller (`rimBackRise` 1.0: rimH × (1 + sin angle), GEO.rimTop gives it), inner rim 0.119, dent 1.12 (a hair below the bed), `holeDirectR` 0.075, side walls re-applied after the bed contacts, and a hard 8 s cap from release, sink included. Straight throws walk 10 → 20 → 30 → 40 → 50 → (over the top) 40 → 30 → 20 → 10 at x0 0 and ±0.3, 100 % seed agreement (`local-dev/skeeball-lab/ladder.js`). Modal from v: 10 2.35, 20 2.6, 30 2.9, 40 3.25, 50 3.6 (centre).
  Bands (σ): 10 98.5 %, 30 99 %, 40 79 %, 50 22 % best / 15 % straight, 100 7.2 %, competent 369, novice 192, cup changed 19.6 %, rattle 18 %, dent 14.7 %, flight 0.52 s, resolve p95 4.56 s / max 7.15 s.
- 2026-09-25 — **Game, player-critic round 1 (rc.10).** Input is read on the whole stage (the thumb rests on the band under the machine) and normalised by the css height of the 384-row machine frame at the current integer scale, not the canvas, so a phone's power no longer shifts with the room rows or the URL bar (same gesture at 390×844 and 375×667: |Δv| ≤ 0.13 %). Power `v = vMin + (vMax − vMin)·clamp((s − 0.15)/(3.9 − 0.15))^2.0` in machine heights/s: the ordinary thumb (≈ 500–1400 px/s) walks the round-5 ladder 10 → 20 → 30 → 40 → 50 and only a hard flick goes over the top. The ball launches from where it was set down (pointerdown x); a slow release (< 0.5 MH/s after > 150 ms) sets the ball down instead of spending it; a hook needs a straight 90 ms before it (three slices within 10°), turns ≥ 35°, reads power/aim from before the hook, and tops out at english 0.4 (≈ 0.16 u of landing). PAYOUT never acts on pointerdown: a touch during the crank runs the tickets out at once and holds 0.5 s; after it, a swipe is a nickel and a throw. **Tokens stay effectively infinite on purpose:** an empty pocket shows the art (door rattles, NO TOKENS on the marquee) and 1.5 s later the machine finds a nickel in its coin return (+5, event `found`, flag `skeeball.found-a-nickel`) — until the changer exists, nobody is locked out. An open game is kept in `Arcade.stats('skeeball').open` and resumed on the next mount.
- 2026-09-25 — **Game decisions rc.3 → rc.11 (for the record).** Aim is read on the screen and converted to the lane: tan(lane aim) = 0.34 · tan(screen angle), because one unit of z at the throw line is 32.5 rows against 96 px of x — the chalk ghost lies under the thumb, and the ±25° clamp is on the lane. The room grows to fill the stage (render `H_MAX` 560, in 8-row steps so the iOS URL bar doesn't rebuild the layers), and the canvas sits on whole device pixels. The mischief lean is 0.03–0.07 u/s² (gentler than §7's 0.06–0.14; see the mischief commit). The refill rule is "NO TOKENS, then the machine finds a nickel" (rc.10 line above). **Seeding:** the harness keeps `opts.seed` / `H.play(seed)` fixed; otherwise each game's seed is `1913·1000003 + stats.games·7919 + stats.lifetimeScore`, so every save plays a different machine (lean, moon, jams) and a given save replays deterministically. A refused ball (the sulk) glides home to the throw line in 0.25 s with no rack lift, and the possum's eyes stay narrowed until it is back. A paused or hidden cabinet takes no input. `?mode=` / `?force=` only work under `?harness=1`.
- 2026-09-25 — **Physics round 6, a propped ball tips.** A slow ball resting on a 100-hole lip is pushed (`lipNudge` 1.2): into the hole if its centre is within lipRing + 0.3·r, otherwise outward and downhill; if that way is blocked for 0.25 s (the painted corner pocket between lip, wall and backstop has no other exit), into the hole. Final positions are clamped inside the walls. `stuck` is now 1.2 % on the best 100 line (x0 0.6, v 7.1, aim 0.12; was 13.9 %) and 0.03 % overall.
  The cost: the corner pocket now feeds the hole. The competent best 100 line is 13.0 % in `metrics` (13.3 % at 1,500 throws; a finer search finds ~18 % at full power from x0 0.6), just over 4–12 %. Other σ bands unchanged; the ladder still holds.
- 2026-09-25 — **The 100 line accepted (coordinator).** The 4–12 % band is retired. New band: best line ≤ 20 %, straight from centre ≤ 8 %. Measured: the full-power corner bank (x0 0.6, v 7.1, aim 0.12) scores 100 13.3 % (≈ 18 % on a finer search); straight from centre peaks at 0.7 %. It is a deliberate skill line. Its round-6 mix over 1,500 throws: 10 24.3 %, 20 6.9 %, 30 1.9 %, 40 0.1 %, 100 13.3 %, gutter 21.7 %, return 30.6 %, stuck 1.2 %. Physics is final unless the last player pass sends a finding.
- 2026-09-25 — **Game, player-critic round 2 (rc.14).** The gesture tops out at `V_TOP` 7.1 (the §11 corner line's v) with `SPEED_CEIL` 3.78 MH/s, so any flick harder than ≈ 2180 px/s is exactly v 7.1. **Measured against physics 08a4018 (+ the uncommitted `backstopGripU` 0):** at x0 0.6 / lane aim 0.12 the 100 is not at v 7.1 — it is 15 % at v 6.6, ≈ 0.7 % at 6.8–7.2 (≈ 50 % returns), and 11 / 29 / 58 % at 7.3 / 7.4 / 7.5, above `vMax`. §11's 13.3 % for the "v 7.1" line comes from the σ 4 % tail above 7.2, which no gesture can reach. With real corner gestures (saturated, ±2° screen, ±3 art px) the corner line scores 0 % competent / 0.3 % novice at V_TOP 7.1; the near-centre line (art x 120, +12°) 8.3 % / 6.8 %. Raising `V_TOP` to 7.35 would make the corner 25 % / 18 % (centre 17 % / 16 %) — one constant, but it is a physics-band decision, so it is left at the requested 7.1. Also: a tail turned 20–35° after a straight run no longer re-aims (power and aim come from before it; english only past 35°); a ball that rolls home (`return` / `bounceback`) shows a "0" toast where it stops; power is normalised by max(machine css height, 384 px), so a landscape phone isn't a hair trigger.
  **Recorded decisions:** the physics resolve cap stays at 8 s (PLAN §9 said 6 s; no real throw in the critics' ~120 real-touch balls exceeded 5.1 s, and the cap is a safety net, not a feel number). **The first payout of a fresh save always jams** (the first game's seed is the constant `1913·1000003`) — kept on purpose: it is the machine introducing itself, and TAP blinks at the slot.
- 2026-09-25 — **Physics round 7 not landed; the 100 geometry accepted as it plays (coordinator).** Six approaches (grippier backstop, rebound-only direct hits, no direct-hit rule, taller down-bed lips, low-friction lips, lips facing the middle) all move the corner bank and the near-centre diagonal together: from x0 ≈ ±0.25 a diagonal reaches a 100 hole by the same route as the corner bank (across the bed, off the side wall, onto the lip). Measured at rc.14 with real gestures and competent noise: corner bank 2–7 % 100 with 29–47 % bouncebacks; near-centre hard diagonal (x0 0, aim 0.24, v 7.1) 12.5 %; straight from centre 0.5 %. **Decision:** ship as is — the 100 lives up the right- or left-hand diagonal and a competent thumb finds it about one throw in eight, which is inside the "best line ≤ 20 %" band; the corner bank is no longer the documented shot. **Open for the owner:** a hood on the inner side of each 100 hole (visible art + physics) would make the corner bank the only route; that changes the painted machine, so it is the owner's call. Also open: knife-edge slivers on a 0.01-v grid (a 50 at v 3.26, a 40 at 3.60; `ladder.js --fine`) — balls landing exactly on a rim top; removing them needs a different rim-top deflection model, not a nudge.
- 2026-09-25 — **The carry (rc.22).** A pointerdown on the resting ball (sprite ± 14 css px) picks it up; the ball's lane position follows the thumb exactly (`laneAt`, a bisection on `R.project`'s sprite row, round-trips to 0 px), with the grab offset kept and no smoothing; pulling back stops it at the throw line. It leaves the hand where the thumb crosses the release line **z = 2.4** (≈ 90 css px of thumb from the ball at 390 wide), at the hand's speed there — the same MH/s → v curve as the swipe (no refit needed: the carry sweep reads gutter → 10 → 20 → 30 → 40 → 50 → over the top, 20/20 seeds), launched at `P.vForLadder(2.4, v)` (0.7–7.2 % under the hand's v). A natural flick rolls free 0.30 s before the hop. Let go before the line: thrown from where it is, or — slower than 0.5 MH/s — set down (it rolls back to the line over 0.4 s, no ball spent). English is a bowler's wrist: 0.6·flick (the last 40 ms against the swing's line, dead zone 12°, full 40°) + 0.7·curl (the swing's second half against its first, 14°/34°), capped ±1; a thumb's natural arc gives 0, a 110 px-radius curl 0.49. The swipe from elsewhere is unchanged.
- 2026-09-26 — **The carry, round 2 (rc.24): the ball leaves your hand the moment you outrun it.** Each move the hand's launch speed is `vL = vForLadder(z, v(hand MH/s, last 40 ms))`; the ball stays under the thumb only while the thumb's lane speed ≤ vL (≈ 90 css px/s at the throw line, 60 at z 2.4), then rolls away from where it is at the speed it had in the hand (×1.2 at most), and while the thumb is still down and speeding up the hand keeps pushing: its speed follows the hand's vL up, never more than ×1.2 per 1/60 s (no speed step: the film shows per-frame steps growing 0.02 → 0.09 z smoothly). On the lift the throw is settled: power is the hand's FASTEST 40 ms going up (a hand can push a ball, never pull it back) — the swipe uses the same rule, so the same gesture is within 1 % either way; aim is the opening 30 % of the swing; english 0.6·flick (the last 40 ms against the swing's line, 15°/45°) + 0.5·curl (second half against first, 22°/44°). A trailing pointerup that repeats the last spot within 25 ms is dropped (same ring at +0/+8/+16 ms). A hand that stops before lifting, having never pushed past 1.12·vMin, sets the ball down (it rolls back); a cancel calls it back the same way. Power curve refit for the peak rule: `SPEED_FLOOR 0.55, SPEED_CEIL 5.4 MH/s, GAMMA 1.4` (ladder ordered, 5/5 per row). z 2.4 remains a backstop for a slow drag (drawn as the chalk line). **Physics request:** the lane hook is ~half what feel wants (spin 0.3 moves the landing 0.08 u at v 3.5, spin 1 0.25 u); `spinA` ≈ 0.55 gives 0.15 / 0.47.
- 2026-09-26 — **The carry, round 3 (rc.25): the ball stays under your thumb until you throw it.** In the hand the thumb sets a target and the ball rolls toward it at up to max(vL, 2.2 u/s): it sits under a slow thumb, lags a quicker roll, stops under a stopped thumb and never gets ahead of it; it can't be carried past z 2.4 (the chalk line is now that limit). A flick throws it — the thumb's speed UP THE LANE over 40 ms passes 1.0 MH/s (≈ 580 css px/s) — struck from where it is at max(its own speed, 0.6·vL), pushed toward the hand's vL ×1.35 per frame only while the thumb is in contact (within 20 css px, or 120 ms of the leave); what's owed on the lift arrives within 2 frames. A lift without a flick sets the ball down where it is (under or just behind the thumb). The swing for aim and english starts after the last pause, corner or backward move (a sideways slide or a hold is not part of the throw); aim is the chord over the first 60 % less a 12° (screen) dead zone for a straight-intended swing, blending to the opening 30 % for a deliberate curl (≥ 20° of curl) so a curl leaves on its opening line and bends in; english dead zones flick 21°, curl 22°. **Physics note (S4):** a throw of 2.30–2.31 u/s from the line (crest speed 1.676–1.69) is a gutter inside the 10 band — the first bed contact sends the ball back over the lip (`bed > gutter`), at any z0.
- 2026-09-26 — **The carry, final (rc.26).** Aim for a straight-intended swing is now `chord60 − 0.6·curl` with a 3° (screen) straight zone, and a bow whose whole chord is straight throws straight: a carried diagonal aims exactly as the same swipe (6–48°, Δ 0.00°), a 250 px-radius thumb arc still lands a 50. A gentle lift throws: if the ball is still rolling in the hand (≥ 0.8·2.2 u/s) and the thumb was going up ≥ 0.6 MH/s over its last 40 ms, it goes at max(its speed, the swipe's power) — a steady 400–500 px/s push lifted anywhere is a 10, where it used to be set down; a ball at rest under a stopped thumb is still set down. The ball eases into the hand (speed ∝ gap, 0.08 s) instead of halting in one frame, and the carry's roll sound reads max(the ball's speed, half the thumb's).
