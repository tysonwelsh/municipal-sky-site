...
window.KolobAudio = (function () {
  ...
  function planMeeting() {
    ...
    // the old tune joins the forced pool at ~weight 1 vs 2/2 for the others
    // (MERGE NOTE: if forcedType becomes a pickW, fold in ["oldtune", 1] and
    // drop this override)
    if (forceVisitation && chance(0.2)) forcedType = "oldtune";
  ...
  function conductorTick() {
    ...
    for (var vv = 0; vv < C.visitations.length; vv++) {
      ...
      if (!V.fired && V.section === C.section && x > 0.2 && x < 0.55 &&
        ...
        V.fired = true;
        // type → set piece (MERGE NOTE: union this map with any other agent's)
        var VISIT_FN = { question: unansweredQuestion, bands: twoBandsCross, oldtune: oldTuneRemembered };
